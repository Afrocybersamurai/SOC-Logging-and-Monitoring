...
> In Event Viewer we can see IP addresses, but we have no idea the location the attacks came from.``` 

- We will first download Two customised geolocation IP files 

![image](https://user-images.githubusercontent.com/112146207/231026643-50acaa3b-3f83-4467-b6ee-49cf28ee2225.png)
 

- Once downloaded we will then go into our Azure account and search "storage account" and click on "Create storage account"
...
- Remember that Redundancy in cybersecurity is often used to improve fault tolerance, which means the ability of a system to continue functioning even in the event a part of it fails.
...
> This might take a while since one of the files is very large, thus patience is essential! 
...
- Change the expiration date and click "generate SAS" then copy the Blob SAS URL and put it on your notes for use later.
...
- Just to explain why we are doing this, we are going to give this SAS URL to the Log repository which will read all this data in Microsoft Sentenial and upload it into its database. 
...
- It's extremely important that you copied the Blob URLs to a notepad because we will use them
...
- For the second watchlist, input this information

![image](https://user-images.githubusercontent.com/112146207/231053907-bdec4f66-2204-4310-8cd4-0ead229d4c39.png)

- Now we have to allow these files to “upload” from our storage account into Sentinel/Log Analytics Workspace.
- The large file will likely take over 24 hours
